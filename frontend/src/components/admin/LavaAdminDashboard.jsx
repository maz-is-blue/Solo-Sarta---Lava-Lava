...
function Field({ item, value, onChange }) {
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      <div style={{ fontSize: 10, letterSpacing: 1.5, color: 'rgba(169,144,204,0.7)', fontFamily: 'DM Sans', marginBottom: 6 }}>
        {item.label}
      </div>
      {isTextarea
      ...
      {(!mobile || sidebarOpen) && (
        <>
          ...
          <div style={{
            ...
            {/* Brand header */}
            <div style={{ padding: '28px 20px 24px', borderBottom: '1px solid rgba(139,111,184,0.1)' }}>
              ...
              <div style={{
                ...
                background: 'linear-gradient(90deg, #E8906A, #A990CC)',
            ...
            <nav style={{ flex: 1, padding: '16px 0' }}>
              {SECTIONS.map(s => (
                <button key={s.key} onClick={() => { setActiveSection(s.key); setSidebarOpen(false) }} style={{
                  ...
                  borderLeft: `2px solid ${activeSection === s.key ? '#A990CC' : 'transparent'}`,
                  fontSize: 11, letterSpacing: 1.5,
                  color: activeSection === s.key ? '#A990CC' : 'rgba(250,248,245,0.45)',
                  transition: 'all 0.2s',
                }}>
                  {s.label.toUpperCase()}
                </button>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {mobile && (
              <button onClick={() => setSidebarOpen(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#A990CC', fontSize: 18, padding: 0 }}>
                ☰
              </button>
            )}
            <div>
              <div style={{ fontSize: 9, letterSpacing: 3, color: 'rgba(169,144,204,0.5)', marginBottom: 2 }}>EDITING</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {saved && (
              <span style={{ fontSize: 11, color: '#A990CC', letterSpacing: 1 }}>✓ SAVED</span>
            )}
            {error && (
            ...
            <button
              ...
              style={{
                ...
                background: saving
                  ? 'rgba(139,111,184,0.4)'
                  : 'linear-gradient(90deg, #E8906A, #8B6FB8)',
