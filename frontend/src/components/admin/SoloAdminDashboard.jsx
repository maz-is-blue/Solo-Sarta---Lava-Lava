...
function Field({ item, value, onChange }) {
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      <div style={{ fontSize: 10, letterSpacing: 1.5, color: 'rgba(201,169,110,0.6)', fontFamily: 'DM Sans', marginBottom: 6 }}>
        {item.label}
      </div>
      {isTextarea
      ...
      {(!mobile || sidebarOpen) && (
        <>
          ...
          <div style={{
            ...
            {/* Brand header */}
            <div style={{ padding: '28px 20px 24px', borderBottom: '1px solid rgba(201,169,110,0.1)' }}>
              <div style={{ fontSize: 9, letterSpacing: 3, color: 'rgba(201,169,110,0.5)', marginBottom: 8 }}>ADMIN</div>
              <div style={{ fontFamily: 'Cormorant Garamond', fontStyle: 'italic', fontWeight: 300, fontSize: 22, color: '#C9A96E' }}>
                Solo Sarto
              </div>
            </div>
            ...
            <nav style={{ flex: 1, padding: '16px 0' }}>
              {SECTIONS.map(s => (
                <button key={s.key} onClick={() => { setActiveSection(s.key); setSidebarOpen(false) }} style={{
                  ...
                  borderLeft: `2px solid ${activeSection === s.key ? '#C9A96E' : 'transparent'}`,
                  fontSize: 11, letterSpacing: 1.5, color: activeSection === s.key ? '#C9A96E' : 'rgba(250,248,245,0.5)',
                  transition: 'all 0.2s',
                }}>
                  {s.label.toUpperCase()}
                </button>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {mobile && (
              <button onClick={() => setSidebarOpen(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#C9A96E', fontSize: 18, padding: 0 }}>
                ☰
              </button>
            )}
            <div>
              <div style={{ fontSize: 9, letterSpacing: 3, color: 'rgba(201,169,110,0.5)', marginBottom: 2 }}>EDITING</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {saved && (
              <span style={{ fontSize: 11, color: '#C9A96E', letterSpacing: 1 }}>✓ SAVED</span>
            )}
            {error && (
              <span style={{ fontSize: 11, color: '#E8906A' }}>{error}</span>
            )}
            <button
              onClick={handleSave}
              disabled={saving}
        ...
        <div style={{ flex: 1, padding: mobile ? '28px 20px' : '36px 48px', maxWidth: 760 }}>
          {sectionItems.length === 0 && (
            <div style={{ color: 'rgba(250,248,245,0.3)', fontSize: 13 }}>Loading content...</div>
          )}
